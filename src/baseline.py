# baseline.py
"""
Non-spiking EEGNet (ReLU) baseline for LOSO comparison against SpikingEEGNet.

Mirrors a single run of the Optuna study (pipeline.py -> experiment_loso_all)
as closely as possible so the comparison isolates ONE factor: spiking (LIF)
vs. continuous (ReLU) activations. Everything else -- architecture shape,
LOSO protocol, dataset, preprocessing, optimizer, epochs, batch size -- is
held identical.

What's reused unchanged from the spiking pipeline:
    constants.py, cache_key.py, load_moabb_dataset.py, make_loader.py

What's different from the spiking pipeline (by necessity, since this model
has no timesteps to aggregate):
    ...
    - No BETA / SPIKE_GRAD_SLOPE / N_STEPS_TRAIN / N_STEPS_EVAL hyperparameters.
    - Architecture (temporal_filters, depth_multiplier, pointwise_filters,
      temporal_kernel_size, separable_kernel_size, pool1_size, pool2_size,
      dropout) is FIXED to the literal EEGNet paper config for the SMR /
      BNCI2014_001 paradigm (Lawhern et al. 2018) -- see LITERAL_EEGNET_SMR_CFG
      below -- rather than pulled from the Optuna study, since that study
      tuned architecture for the spiking model's search space, not this
      baseline. Training hyperparameters (lr, epochs, batch_size) are still
      pulled from the same best_params source as ablation.py so the optimizer
      settings stay comparable across the two runs.
...
def main():
    ...
    print("Resolved config (baseline: ReLU EEGNet, non-spiking):")
